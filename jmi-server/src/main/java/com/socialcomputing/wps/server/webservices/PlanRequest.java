...
public class PlanRequest {
     ...
     * : "entityId" : reference entity "attributeId" : reference attribute for
     * discovery plans "affinityReaderProfile" : force the affinity reader
     * profile "analysisProfile" : force the analysis profile "displayProfile" :
     * force the display profile "appletSize" : dimension of the Applet ( ex:
     * "appletSize=300,400")
     */
    ...
    public PlanRequest(Connection WPSConnection, WPSDictionary dico, Hashtable<String, Object> parameters)
        ...
        m_classifyId = new RequestingClassifyId(
                                                this.getParameter("classifyId") != null ? this
                                                        .getParameter("classifyId") : m_entityId);
                    ...
                    for (int i = 0; st.hasMoreTokens(); ++i) {
                        ...
                        if (pos != -1) {
                            ...
                            if (m_discoveryAttributeId.startsWith(prefix))
                                this.m_RequestParameters.put("~$connectorAttributeId",
                                                             m_discoveryAttributeId.substring(prefix.length()));
                        }
                    }
                }
                else
    ...
    public AnalysisProfile getAnalysisProfile() throws JMIException {
        ...
        if (profile == null) { // Analysis profile is dictionary defined
            m_AnalysisProfile = m_Dictionary.getAnalysisProfile(m_classifyId);
        }
        else { // Analysis profile is applet defined
    ...
    /**
     * Returns the property list of this Group plus a "SELECTION" property
    ...
    public Hashtable<String, Object> getEntityProps(String swatchName, String id) throws JMIException {
        ...
        for (i = 0; i < sCnt; mask <<= 1, ++i) {
            if (model.m_EntitiesSelections[i] != null) {
                iSelectionConnector selCon = m_Dictionary.getEntityConnector()
                        .getSelection(model.m_EntitiesSelections[i].m_SelectionRef);
    ...
    public void updateInterProps(ProtoAttribute att, XSwatch restSwh, XSwatch curSwh, Hashtable props)
        ...
        String name;
        Object prop;
        int i, rCnt = restVals.length, vCnt = rCnt + curVals.length;
    ...
    /**
     * Returns the property list of this Node plus a "SELECTION" property
     * (32bits Integer). The Model find the Swatch corresponding to this id and
    ...
    public void putAttributeProps(Recommendable recommendable, XSwatch restSwh, XSwatch curSwh,
        ...
        // Generator used swatch properties
        ValueContainer[] restVals = restSwh.getProps(), curVals = curSwh.getProps();
        ValueContainer val;
        String[] propNames;
        String name, dbPropertyName;
        Object prop = null;
        int i, j, pCnt, rCnt = restVals.length, vCnt = rCnt + curVals.length;
        // boolean isInner;
        ...
        for (j = 0; j < vCnt; j++) {
            ...
            for (i = 0; i < pCnt; i++) {
                ...
                if (name.charAt(0) == '~')
                    name = propNames[i + 1];// Inner Prop
                if (name.charAt(0) == '$')
                    continue;

                dbPropertyName = profile.getAttributeSwatchProperty(name);
                prop = null;
                if (dbPropertyName == null) { // May be a motor property, look
                                              // at Recommendations
        ...
        for (i = 0; i < sCnt; mask <<= 1, ++i) {
            ...
            if ((sel != null) && !sel.m_FreeSelection) {
                iSelectionConnector selCon = getAnalysisProfile().getConnector(m_Dictionary)
                        .getSelection(model.m_AttributesSelections[i].m_SelectionRef);
    ...
    /**
     * Returns the property list of this Link plus a "SELECTION" property
     * (32bits Integer). The Model find the Swatch corresponding to this id and
     * calls getBoundNames. Those names can be cached by the XSwatch.
     */
    public void putAttLinkProps(Recommendable recommendable, XSwatch restSwh, XSwatch curSwh, Hashtable props)
        ...
        String[] propNames;
        String name, dbPropertyName;
        int i, j, pCnt, rCnt = restVals.length, vCnt = curVals == null ? rCnt : rCnt + curVals.length;
        // boolean isInner;
        ...
        for (i = 0; i < size; i++) {
            ...
            switch (recIndex) {
                ...
                case RecommendationGroup.SATTRIBUTES_RECOM:
                    dbProps = profile.getConnector(m_Dictionary).getSubAttribute()
                            .getProperties(strIds[i], null, m_entityId);
                    break;
            }
            if (dbProps != null) {
    ...
    /**
     * Returns the Swatch of the specified type used by all
     * Nodes/Links/Clusters/Adds or the Reference Object. Groups are segmented
     * so we need to use another methode.
     * 
    ...
    public XSwatch getSwatch(int type, int style) throws JMIException {
        ...
        switch (type) {
            ...
            case XSwatch.LINK_TYP:
                ...
                if (style > 1 && swatchName.equals(WPSDictionary.DEFAULT_NAME))
                    return null;
                break;
    ...
    public XSwatch getSwatch(String swatchName) throws JMIException {
        ...
        if (swatchName != null) {
            ...
            if (swatch == null) {
                try {
                    // SwatchLoader loader =
                    // m_SwatchLoaderHome.findByPrimaryKey( swatchName);
    ...
    static public Object ExtractProperty(String definition, Hashtable<String, Object> properties)
        ...
        for (int i = 0; st.hasMoreTokens(); ++i) {
            ...
            if (item.startsWith("\"") && item.endsWith("\""))
                ...
                // if( value == null)
                // throw new WPSConnectorException( "Property '" + item +
                // "' in '" + definition + "' can't be evaluated ");
                if (value != null)
                    result.append(value);
            }
        }
