...
/**
 * <p>Title: PlanGenerator</p>
 * <p>Description: A generator of graphical Plan that can be displayed with the WPSApplet.<br>
 * A valid protoPlan is needed as input and a Plan and an Env can be retrieved as output.
 * It has also a few debug features like the visualisation and modification of the relaxation process,
 * the saving of a plan (standalone and serialized stream) and a simple test loop.</p>
...
{
	/**
	 * Output type for a raw serialized Env/Plan (not readable by the WPSApplet).
	 */
...
//	protected static final int	SWF_OUT     = 6;    // SWF : Flash4 format
	...
	/**
	 * Generate a new Plan using a ProtoPlan and eventualy display the relaxation process.
	 * Before generating the new Plan, the ProtoPlan datas are process to extract statistical information
	 * and ensure its integrity.
	 * @param plan		A ProtoPlan holding the plan information and a PlanRequest to connect to the DB.
	 * @param isVisual	True to display the relaxation GUI.
	 */
	public void generatePlan( ProtoPlan plan, boolean isVisual ) throws com.socialcomputing.wps.server.plandictionary.connectors.WPSConnectorException
	{
		if ( plan.init())
		{
			plan.evalBounds();
	...
	**/
	 ...
	 * @param planPrm	Parameters of the Plan to generate.
	 * @return			A new ProtoPlan defined by planPrm.
	 */
	private static ProtoPlan preGenerate( PlanParams planPrm ) throws com.socialcomputing.wps.server.plandictionary.connectors.WPSConnectorException
	{
		EZTimer	timer	= new EZTimer();

		planPrm.init();

		//SwatchLoaderHome	swatchLoaderHome	= (SwatchLoaderHome) PortableRemoteObject.narrow( planPrm.m_loader, SwatchLoaderHome.class );
	...
	/**
	 * Post step to save the Plan generated to a file.
	 * The file format depends on the 'output' command line param.
	 * @param planPrm	Parameters of the generated Plan.
	 */
	private void postGenerate( PlanParams planPrm )
	{
		...
		if ( env != null && plan != null )
		{
			...
			{
				case ( SERIAL_OUT ) :   writeObject( env, plan, planPrm, false ); break;
				case ( SERIALZ_OUT ) :  writeObject( env, plan, planPrm, true ); break;
				case ( XML_OUT ) : writeXML(env, plan, planPrm); break;
				default :               System.out.println( "Wrong output type! [0-2]" );
			}
		}
		else
		{
	...
	/**
	 * Writes a couple Env+Plan as a standalone MapStan plan and as a serialized Object compressed or not.
	 * @param env		The Env to Write.
	 * @param plan		The Plan to write.
	 * @param planPrm	Parameters of this plan containing its ID.
	 * @param needComp	True if the serialized WPSApplet Plan should be compressed using GZIP.
	 */
	 ...
	 * @param args
	 * <ul>
	 * <li>debug=[-1,0,1,2]. -1 for no messages, 2 for maximum verbosity.</li>
	 * <li>degenerate=true. Simulates a degenerated Plan (only 1 cluster).</li>
	 * <li>testNet=true. Creates the personal plans of all the mapstan.net users.</li>
	 * <li>visual=true. Open a GUI to display/edit the relaxation process.</li>
	 * <li>output=[0,1]. 0 for raw output, 1 for GZIP compressed output.</li>
	 */
	public static void main( String[] args )
	{
		try {
			...
			EZTimer         timer	= new EZTimer();
			boolean			testDeg		= s_params.isEnabled( "degenerate" ),
							testNet		= s_params.isEnabled( "testNet" ),
							isVisual	= s_params.isEnabled( "visual" );
			PlanParams[]	params	= new PlanParams[]
			{
				new PlanParams( "charon", "Boosol", AnalysisProfile.PERSONAL_PLAN, 1 ),		// 0 flugue Plan
				new PlanParams( "charon", "SEngine", AnalysisProfile.PERSONAL_PLAN, 1 ),	// 1 mapstan
				new PlanParams( "charon", "SEngine", AnalysisProfile.PERSONAL_PLAN, 12 ),	// 2 amiga ppc
				new PlanParams( "charon", "SEngine", AnalysisProfile.DISCOVERY_PLAN, 25 ),	// 3 mapstan.net
				new PlanParams( "charon", "SEngine", AnalysisProfile.DISCOVERY_PLAN, 1 ),	// 4 mapstan.com
				new PlanParams( "charon", "SEngine", AnalysisProfile.DISCOVERY_PLAN, 4 ),	// 5 mapstan.net
				new PlanParams( "charon", "SEngine", AnalysisProfile.DISCOVERY_PLAN, 250 ),	// 6 linternaute
				new PlanParams( "charon", "SEngine", AnalysisProfile.PERSONAL_PLAN, 31206 ),// 7 Amiga PPC
				new PlanParams( "charon", "SEngine", AnalysisProfile.PERSONAL_PLAN, 59863 ),// 8 mapstan search
				new PlanParams( "charon", "Boosol", AnalysisProfile.PERSONAL_PLAN, 2 ),		// 9 espinat Plan
				new PlanParams( "charon", "Boosol", AnalysisProfile.GLOBAL_PLAN, 6227 ),	// 10
				new PlanParams( "charon", "SEngine", AnalysisProfile.PERSONAL_PLAN, 27 ),	// 11 bug
			};
			EZDebug.setVerbosity( s_params.getIntParameter( "debug", EZDebug.NONE ));
			...
			else
			{
				...
				do
				{
					...
					protoPlan	= preGenerate( planPrm );
					timer.showElapsedTime( "init " + planPrm.getPlanName(), EZDebug.MED );
					...
					planGenerator.postGenerate( planPrm );//getIntValue( "output", SERIALZ_OUT ));
...
/**
 *
 * <p>Title: PlanParams</p>
 * <p>Description: This simple container holds the parameters used to create retrieve a ProtoPlan.</p>
 * <p>Copyright: Copyright (c) 2001-2003</p>
 * <p>Company: MapStan (Voyez Vous)</p>
 * @author flugue@mapstan.com
 * @version 1.0
...
class PlanParams
{
	...
	/**
	 * Creates a new PlanParams knowing the WPS server, the name and type of the Plan and the entity/attribute id.
	 * @param server	WPS Server URL.
	 * @param name		Plan name (SEngine, Boosol...)
	 * @param type		Plan type (AnalysisProfile.XXX_PLAN)
	 * @param id		Id of the entity/attribute to make a Plan about.
	 */
