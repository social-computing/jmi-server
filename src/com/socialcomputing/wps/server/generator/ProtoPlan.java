...
public class ProtoPlan
{
	...
	/**
	 * A link Comparator to sort links from the best to the worst.
	 * First are the base links then the mixed one and then the ext links.
	 * In each category, links are sorted accordingly to their size/length ratio and filtering state.
	 * So the thick short links are before the tight long ones that are before the filtered ones.
	 */
	protected static Comparator  s_linkCmp   = new Comparator()
	{
		public int compare( Object o1, Object o2 )
		{
			AttributeLink   l1  = (AttributeLink)o1,
							l2  = (AttributeLink)o2;

			if ( l2.isBase())
			{
				if ( l1.isBase())		return	compareSizeAndLength( l1, l2 );
				else					return 1;
			}
			else if ( l2.isMixed())
			{
				if ( l1.isMixed())		return	compareSizeAndLength( l1, l2 );
				else if ( l1.isBase())	return -1;
				else					return 1;
			}
			else // l2.isExt())
			{
				if ( l1.isExt())		return	compareSizeAndLength( l1, l2 );
				else					return -1;
			}
		}
	};
	 ...
	 * @param planReq	The Plan Request corresponding to this.
	 */
	...
	public String getXML()
	{
		...
		for ( i = 0; i < n; i ++ )
		{
			...
			result += "<string builtin='elementType'>"+attr.m_strId+"</string>\n";
			result += "<string builtin='elementSize'>"+attr.m_size+"</string>\n";
			result += "<string builtin='isBase'>"+attr.m_isBase+"</string>\n";
			...
			while (iter.hasNext())
			{
				...
				{		
					...
					result += "<string builtin='"+key+"'>"+value.toString()+"</string>\n";
			}			
		...
		for ( i = 0; i < n; i ++ )
		{
			...
			if (link.m_isWeak) continue;
			if (w<=1.0) continue;
			
			result += "<link id='b"+i+"'>";
			result += "<string builtin='atomRef'>"+link.m_from.m_strId+"</string>\n";
			result += "<string builtin='atomRef'>"+link.m_to.m_strId+"</string>\n";
			result += "<string builtin='order'>"+w+"</string>\n";

			Collection coll = link.getRecommendations(RecommendationGroup.ENTITIES_RECOM);
			String[] c= (String[])link.getRecommendations(RecommendationGroup.ENTITIES_RECOM).toArray( new String[0] );
			if (c.length!=0)
			{
		...
		for ( i = 0; i < n; i ++ )
		{
			...
			result += "<link ";
			result += "from='"+link.m_from+"' ";
			result += "to='"+link.m_to+"'";
			result += "size='"+link.m_size+"'";
			result += " />\n";
		}*/
	...
	{
		if ( n > 0 )    EZDebug.println( String.valueOf( n )+ " " + msg, EZDebug.HIGH );

		return n > 0;
	}

	/**
	* Compares both size and length as a ratio.
	* Filtered links are always worst than the others.
	* @param l1	A link in the list to sort.
	* @param l2 A link different from l1 in the list to sort.
	* @return	1 if l2 is better than l1, -1 for the opposite and 0 if they are equal.
	*/
	private static int compareSizeAndLength( AttributeLink l1, AttributeLink l2 )
	{
		if ( l2.isFiltered() == l1.isFiltered())
		{
			float	r1	= l1.m_size / l1.m_length,
					r2	= l2.m_size / l2.m_length;
			return r2 > r1 ? 1 :( r2 < r1 ? -1 : 0 );
		}
		else	return l2.isFiltered() ? -1 : 1;
	}

	// Compare length first then size
/*	private int compareLength( AttributeLink l1, AttributeLink l2 )
	{
		if ( l2.isFiltered() == l1.isFiltered())
			return l1.m_length > l2.m_length ? 1 :( l1.m_length < l2.m_length ? -1 : l2.m_size - l1.m_size );
		else	return l2.isFiltered() ? -1 : 1;
	}

	// Compare size first then length
	private int compareSize( AttributeLink l1, AttributeLink l2 )
	{
		if ( l2.isFiltered() == l1.isFiltered())
			return l2.m_size > l1.m_size ? 1 :( l2.m_size < l1.m_size ? -1 : l1.m_length - l2.m_length );
		else	return l2.isFiltered() ? -1 : 1;
	}*/
	...
	protected void degenerate()
	{
		...
		for ( i = 0, j = 1; i < n; i ++ )
		{
			if ( m_attributes[i].m_parent == att )	m_attributes[j++]	= m_attributes[i];
		}
	...
	/**
	 * Check Plan integrity and filter links to keep only the most interesting for the relaxation.
	 * @return	True if this Plan is valid (at least one cluster and if more than one, no cluster without link).
	 */
	protected boolean init() throws com.socialcomputing.wps.server.plandictionary.connectors.WPSConnectorException
	{
		...
		for ( i = 0; i < n; i ++ )
		{
			...
			if ( att.m_parent == null )
			{
				...
				if ( att.m_links == null )
				{
					EZDebug.println( "NULL LINKS ATT " + att, EZDebug.MED );
					nullLinks ++;
				}
				else
				{
					...
					{
						EZDebug.println( "NO LINK ATT " + att, EZDebug.MED );
						voidLinks ++;
					}
					else
					{
				...
				if ( att.m_isRef )  refNodes ++;
			}
			else
			{
				if ( att.m_links != null && att.m_links.size()> 0 )
				{
					EZDebug.println( "LINK IN CLUST.ATT " + att, EZDebug.MED );
					extraLinks ++;
				}
				if ( att.m_children != null )
				{
					EZDebug.println( "CHILD IN CLUST.ATT " + att, EZDebug.MED );
					extraChild ++;
				}
			}
			...
			if ( isInBase )
			{
				if ( isBase )   m_baseAttCnt ++;
				else            isInBase    = false;
			}
			else if ( isBase )  extraBase ++;
		}

		if ( nodeLinks % 2 != 0 )   EZDebug.println( "Odd number of referenced links in nodes : " + nodeLinks, EZDebug.HIGH );

		EZDebug.println( "attributes = " + n, EZDebug.MED );
		EZDebug.println( "ref atts = " + refNodes, EZDebug.MED );
		EZDebug.println( "base atts = " + m_baseAttCnt, EZDebug.MED );
		EZDebug.println( "parent atts = " + clustered, EZDebug.MED );
		...
		for ( i = 0; i < n; i ++ )
		{
			...
			{
				EZDebug.println( "DEAD LINK " + link, EZDebug.MED );
				deadLinks ++;
			}

			if ( !link.isValid())
			{
				EZDebug.println( "INVALID LINK " + link, EZDebug.MED );
				invalidLinks ++;
			}
...
//				if ( !link.m_isFiltered )
				{
					...
					if ( isInBase )
					{
						if ( isBase )       m_baseLinkCnt ++;
						else                isInBase    = false;
					}
					else if ( isBase )
					{
						EZDebug.println( "EXTRA BASE LINK " + link, EZDebug.MED );
						extraBase ++;
					}

					if ( link.m_from.m_parent != null || link.m_to.m_parent != null )
					{
						EZDebug.println( "JUNK LINK " + link, EZDebug.MED );
						junkLinks ++;
		...
		EZDebug.println( "links = " + n, EZDebug.MED );
		EZDebug.println( "base links = " + m_baseLinkCnt, EZDebug.MED );
		...
		if ( nodeLinks != m_attLinks.length )   EZDebug.println( "Different number of links in nodes ( " + nodeLinks + " ) and link table ( " + m_attLinks.length + " )", EZDebug.HIGH );
	...
	public void evalBounds()
	{
		if ( !m_isDegenerated )
		{
			...
			m_bounds[A_ALLSIZE_BND].setBounds( Math.min( m_bounds[A_SIZE_BND].m_min, m_bounds[AB_SIZE_BND].m_min ), Math.max( m_bounds[A_SIZE_BND].m_max, m_bounds[AB_SIZE_BND].m_max ));
			...
			float	widthMin	= Math.min( m_bounds[LB_WIDTH_BND].m_min, m_bounds[LM_WIDTH_BND].m_min ),
					widthMax	= Math.max( m_bounds[LB_WIDTH_BND].m_max, m_bounds[LM_WIDTH_BND].m_max ),
					lenMin		= Math.min( m_bounds[LB_LENGTH_BND].m_min, m_bounds[LM_LENGTH_BND].m_min ),
					lenMax		= Math.max( m_bounds[LB_LENGTH_BND].m_max, m_bounds[LM_LENGTH_BND].m_max );

			m_bounds[L_ALLWIDTH_BND].setBounds( Math.min( m_bounds[L_WIDTH_BND].m_min, widthMin ), Math.max( m_bounds[L_WIDTH_BND].m_max, widthMax ));
			m_bounds[L_ALLLENGTH_BND].setBounds( Math.min( m_bounds[L_LENGTH_BND].m_min, lenMin ), Math.max( m_bounds[L_LENGTH_BND].m_max, lenMax ));

			if ( EZDebug.isPrintable( EZDebug.LOW ))
			{
				n = s_bndNames.length;

				for ( i = 0; i < n; i ++ )
				{
					EZDebug.println( s_bndNames[i] + " bounds = " + m_bounds[i], EZDebug.LOW );
				}
			}
		}
	}
	...
	/**
	 * Equalize an histogram from a value knowing its bounds and eventualy display it.
	 * The given bound ID is also used to retrieve the values to equalize.
	 * @see				#renormalizeHisto
	 * @param boundId	Index of the bound in the bound table.
	 * @param g			A Graphics to draw into or null to avoid visual feedback.
	 */
	protected void EQHisto( int boundId, Graphics g )
	{
		int[]		histo		= new int[10];
		int			i, n		= histo.length,
					max, j 		= 0;
		Bounds		bounds		= m_bounds[boundId],
					histoBnds	= new Bounds( 0, n - 1 );
		...
		if ( bounds.getWidth() > 5 )
		{
			try
			{
				...
				if ( g != null )
				{
					int	histoWidth	= 200,
						...
						dy			= 10,
						x, y		= 0,
						h, w		= (int)Math.ceil( histoWidth /(float)n );
					...
					g.drawString( s_bndNames[boundId] + bounds, dx, dy + histoHeight + 20 );
				}
			...
			{
				e.printStackTrace();
				EZDebug.println( "displayHisto [" + boundId + "] error : " + e, EZDebug.HIGH );
			}
		}
	}
	 ...
	 * @param histo		An histogram array containing the number of value for each intervals.
	 * @param objs		An array of Objects holding the value to renormalize matching that generates histo.
	 * @param field		The value of the Objects to transform.
	 * @param bounds	Min and max of the values to remap.
	 * @param hMax		Maximum number of value in an interval of the histogram.
	 * @throws IllegalAccessException
	 */
	private void renormalizeHisto( int[] histo, Object[] objs, Field field, Bounds bounds, int hMax )
	throws IllegalAccessException
	{
		int		i, j, n	= histo.length,
				min		= (int)bounds.m_min;
...
//		if ( pow < 0.f )		pow	= 0.f;	// collapse small values!
		...
		EZDebug.println( "filtering " + bounds + " using a " + histo.length + " values histogram", EZDebug.MED );
		bounds.m_max	= q;
		...
		EZDebug.println( "new max bound is " + bounds.m_max, EZDebug.MED );
	}

	/**
	 * Returns a node Swatch from the server with a specified style.
	 * @param style	an index to identifies a variant of a swatch:
	 * <ul>
	 ...
	 */
	protected XSwatch getNodeSwatch( int style ) throws com.socialcomputing.wps.server.plandictionary.connectors.WPSConnectorException
	{
	 ...
	 */
	protected XSwatch getLinkSwatch( int style ) throws com.socialcomputing.wps.server.plandictionary.connectors.WPSConnectorException
	{
		return m_planReq.getSwatch( XSwatch.LINK_TYP, style );
	}

	/**
	 * Filters links to keep only the most interesting for the relaxation.
	 * This magical formula should be teleported in the Analysis process...one day...
	 * First, links are separated in 3 categories : base, mixed and ext and counted.
	 * Then the same is done with attributes and their inner links table.
	 * Each attribute link table is sorted so base is first then mix and ext.
	 * In each subcategory, links are sorted accordingly to their 'importance'.
	 * Second, for each attributes, the best links are choosen.
	 * The number of links to choose depend on the XXXLinkMin Dictionary settings.
	 * Then all relaxable links of the Plan are sorted accordingly to their 'importance'.
	 * To ensure not too much links were created by the second step,
	 * try to remove the links in base attributes that are more numerous than baseLinkMax Dictionary settings.
	 * If parts of the base are separated, add necessary links to ensure connexity.
	 * Finaly, adds the best links of each categories.
	 * The number of links to add depends on the XXXLinkKeep Dictionary settings.
	 * @see	#s_linkCmp
	 */
	private void filterLinks()
	{
		int             i, j,
						m, n	= m_relaxLinks.length,
						baseCnt = 0,
						mixCnt  = 0,
						extCnt  = 0;
		AttributeLink   link;
		...
		// Eval the max|min number of base|mixed|ext links / att. And count the base|ext atts
		for ( i = j = 0; i < n; i ++ )
		{
			att    = m_attributes[i];
			...
			if ( att.m_parent == null )
			{
				if ( att.isBase())	attBaseCnt ++;
				else				attExtCnt ++;

				// Sort the links of the atts and count the different types (base, mixed, ext)
		...
		{
			link	= m_relaxLinks[i];
			if ( !link.m_isFiltered && !link.isMixed())	link.updateSupernumerary();
		}

		// Find the link filtered that shouldn't if we want the base to stay connex then "unfilter" them.
		for ( i = 0; i < baseCnt; i ++ )
		{
			link	= m_relaxLinks[i];
			if ( link.m_isFiltered && !link.isLinkFiltrable( this ))	link.m_isFiltered = false;
		}
	 ...
	 * @param isMarked	True to enable the marker, false to disable it.
	 */
