...
/**
 * <p>Title: Mapper</p>
 * <p>Description: This manages the Plan generation through different stages.<br>
 * The stages are:
 * <ul>
 * <li>Base relax : Relaxation of the base without much constraints to give more freedom to attributes.</li>
 * <li>Base filter : Quick relaxation of the base with higher constraints to avoid covering and intersections.
 * 					This stage give the final position of the base attributes so they are locked.</li>
 * <li>Ext relax : Relaxation of the external attributes without much constraints to give more freedom to attributes.
 * 					The external attributes are repulsed by the circle surrounding the base.</li>
 * <li>Ext filter : Relaxation of the external attributes with more constraints.</li>
 * <li>Ext last : Relaxation of the external attributes with highest constraints to avoid covering and intersections.
 * 					The external attributes are repulsed by the base attributes, not the surounding circle.</li>
 * <li>Tesselation : Delaunay triangulation is applied to creates new links including fake ones.</li>
 * <li>Zonisation : Converts this graphical objects to their WPSApplet zone equivalent. This is not a graphical stage.</li>
 * <li>init swatchs : Retrieve swatchs and properties for each zone created. This is not a graphical stage.</li>
 * </ul>
 * </p>
...
public class Mapper
{
	 ...
	 */
	private static final String[]   s_stages    = new String[]
	{
		"Base relax",
		"Base filter",
		"Ext relax",
	...
	/**
	 * Plan data given by the Analysis.
	 * This data is used to create this Mapper.
	 */
	protected   ProtoPlan           m_protoPlan;

	/**
	 * WPSApplet Plan equivalent to this or m_protoplan.
	 * Raw analysis data = m_protoPlan ==> graphical relaxation data = this Mapper ==> WPSApplet final data = m_plan
	 */
	...
	/**
	 * A reference to a Cross manager that will do the real intersection detection / uncrossing job.
	 */
	...
	/**
	 * Current last node to relax.
	 * That means all nodes of the table are relaxed from the first to the m_curNode one.
	 */
	...
	/**
	 * Creates a new Mapper using a ProtoPlan.
	 * This initialize graphical data according to the ProtoPlan data.
	 * @param plan	A ProtoPlan holding the Attributes and Links used by the Mapper.
	 */
	 ...
	 * @return	The name of the current stage or "" if the current stage is not defined.
	 */
	...
	public void generatePlan() throws com.socialcomputing.wps.server.plandictionary.connectors.WPSConnectorException
	{
		...
		setIntrinsicProps();
		timer.showElapsedTime( "All stages", EZDebug.HIGH );

		initNodesSwatchs();
		timer.showElapsedTime( "initNodesSwatchs", EZDebug.HIGH );

		if ( !m_protoPlan.m_isDegenerated )
		{
			initLinksSwatchs();
			timer.showElapsedTime( "initLinksSwatchs", EZDebug.HIGH );
		}
	...
	/**
	 * Initialize graphical nodes and links.
	 * Sorts nodes using their weight.
	 * @param winBnds	Not used in this implementation.
	 */
	...
	/**
	 * Initialize graphical data and execute all graphical stages.
	 * For each stage, execute all the steps.
	 * For each steps, execute all the iterations.
	 */
	...
	private void run()
	{
		...
		while ( initStage())
		{
			...
			timer.showElapsedTime( getStage(), EZDebug.HIGH );
		}
	...
	/**
	 * Initialize data specific to the current stage (counters...).
	 * The current stage is first incremented.
	...
	/**
	 * Initialize data specific to the current step (counters...).
	 * The current step is first incremented.
	 * @return	False if the current step has reach its maximum (to go to the next stage).
	 */
	 ...
	 * @return	False if the iteration as reach its maximum or the relax error is less than its treshold.
	 */
	...
	public void iterate( EZFlags flags )
	{
		switch ( m_stage )
		{
			...
			case TESSELATION_STG :
				try {
					if ( m_step	== 0 )	tesselate();
					else				normalizeData();
				}
				catch( com.socialcomputing.wps.server.plandictionary.connectors.WPSConnectorException e)
				{
					e.printStackTrace();
				}
				break;
...
//=====================================================================================================
	...
	/**
	 * Initialize the Mapper node table by creating an equivalent of each cluster attributes.
	 * It also evaluate the number of base clusters.
	 */
	...
	/**
	 * Initialize the Mapper link table by creating an equivalent of each AttributeLinks.
	 */
	...
	/**
	 * Tesselate the nodes and adds the resulting links to the existing ones if they don't intersect.
	 * Fake links are also added and stored in m_fakeLinks, by creating fake nodes outside screen before tesselation.
	 * Before tesselating, all remaining crossing links are filtered.
	 * New links are added only if they exists in the ProtoPlan and the angle between them and previous links is > PI/8.
	 */
	protected void tesselate() throws com.socialcomputing.wps.server.plandictionary.connectors.WPSConnectorException
	{
		...
		Bounds			widthBnd	= m_protoPlan.m_bounds[ProtoPlan.L_ALLWIDTH_BND],
						lenBnd		= m_protoPlan.m_bounds[ProtoPlan.L_ALLLENGTH_BND];
		boolean			isFakeFrom, isFakeTo;
		//Node			fromNode, toNode;
		float			alpha		= EZMath.PI / 8;	// Minimal angle between Triangulation and relaxation links
		...
		for ( i = m = 0; i < n; i ++ )
		{
			...
			if ( !isFakeFrom && !isFakeTo )
			{
				...
				if (( linkDat == null )//|| linkDat.isDead())  // this link doesn't already exists or has been removed
					&& m_crossMgr.hasOneInterMax( fromDat, toDat, links, inters ))
				{
					...
					if ( attLink != null )	// this link exists but is too weak to be relaxable, let's add it.
					{
						j		= inters[0];
	...
	/**
	 * Renormalize graphics value of nodes and links by equalizing their histogram.
	 * This transformation keep order but lessen differences between high and low values.
	 * Link width and lenght and Node size are transformed.
	 */
	...
	/**
	 * Creates the WPSApplet Plan in the degenerate case (only one cluster in the Plan).
	 */
	...
	/**
	 * Creates the WPSApplet Plan using the Mapper nodes and links.
	 * Links are sorted using the LinkMapData.s_comp comparator.
	 */
	protected void toZones() throws com.socialcomputing.wps.server.plandictionary.connectors.WPSConnectorException
	{
		int         i, j, n	= m_nodes.length,
					m		= m_fakeLinks.length;
		//ActiveZone  zone;
		//NodeMapData  node;
  ...
  /**
	* Initialize the node Swatchs and properties.
	* This includes the WPSApplet BagZones (cluster attributes) and ActiveZones (clusterized attributes).
	* While the "normal" properties are set, the "inter" properties bounds are evaluated.
	* Then the automatic "inter" properties can be set.
	*/
	protected void initNodesSwatchs() throws com.socialcomputing.wps.server.plandictionary.connectors.WPSConnectorException
	{
		EZDebug.push();
		EZTimer         timer = new EZTimer();
		//NodeMapData      sNode;
		ProtoAttribute  att;
		int             i, m	= m_nodes.length,
						n 		= m_plan.m_nodes.length;
		XSwatch[]       nodeSSwh;
		XSwatch[][]     serverSwatchs       = new XSwatch[][]
		{
			{   // Normal Node ServerSwatchs
				m_protoPlan.getNodeSwatch( 0 ),
				m_protoPlan.getNodeSwatch( 2 )
			},
			{   // Reference Node ServerSwatchs
				m_protoPlan.getNodeSwatch( 1 ),
				m_protoPlan.getNodeSwatch( 3 )
			}
		};
		...
		Swatch[]        nodeSwh;
		Swatch[][]      clientSwatchs   = new Swatch[][]
		{
			{   // Normal Node ClientSwatchs
				(Swatch)serverSwatchs[0][0].toClient(),
				(Swatch)serverSwatchs[0][1].toClient()
			},
			{   // Reference Node ClientSwatchs
	...
	*/
	protected void initLinksSwatchs( ) throws com.socialcomputing.wps.server.plandictionary.connectors.WPSConnectorException
	{
		EZDebug.push();
		EZTimer         timer = new EZTimer();
		LinkMapData      sLink;
		...
		XSwatch[][]     serverSwatchs   = new XSwatch[][]
		{
			{   // Normal Link ServerSwatchs
				m_protoPlan.getLinkSwatch( 0 ),
				m_protoPlan.getLinkSwatch( 2 ),
			},
			{   // Reference Link ServerSwatchs
				m_protoPlan.getLinkSwatch( 1 ),
				m_protoPlan.getLinkSwatch( 3 ),
			}
		};
		...
		Swatch[][]      clientSwatchs   = new Swatch[][]
		{
			{   // Normal Node ClientSwatchs
				(Swatch)serverSwatchs[0][0].toClient(),
				serverSwatchs[0][1] == null ? null : (Swatch)serverSwatchs[0][1].toClient()
			},
			{   // Reference Node ClientSwatchs
				(Swatch)serverSwatchs[1][0].toClient(),
