...
public class JDBCProperties implements java.io.Serializable
{
	...
	private static final long serialVersionUID = -5574991072534266245L;
        ...
        {   // Une requ�te SQL permettant d'extraire des propri�t�s
                ...
                private transient Vector m_ColumnInfo = null;
                        ...
                        try {
                                ...
                                if( ret)
                                {
                                        ...
                                        {   // Recherche des types
                                                ...
                                                m_ColumnInfo = new Vector( count);
                                                for( int i = 1; i <= count; ++i)
                                                {
                                                				//System.out.println("m_bNameIsLabel("+i+") :"+m_bNameIsLabel);
                                                        m_ColumnInfo.add( new InternalColumnInfo( m_bNameIsLabel ? rsmd.getColumnLabel( i) : rsmd.getColumnName( i), rsmd.getColumnType( i)));
                                                }
                                        }
                                }
                                else
                                        ;//System.out.println( "JDBCPropertyGroup failed to find column information for " + type + " '" + id + "'");
                        }
                        catch( SQLException e)
                        {
                                throw new WPSConnectorException( "JDBCPropertyGroup failed to find column information for " + m_Type + " '" + id + "'", e);
                        }
                        return ret;
                }
                ...
                private Object getProperty( ResultSet rs, int index) throws WPSConnectorException
                {
                        ...
                        try
                        {
                                ...
                                Object o = null;
                                info = ( InternalColumnInfo) m_ColumnInfo.elementAt( index);
                                ...
                                switch( info.m_Type)
                                {
                                        ...
                                        case java.sql.Types.CHAR:
                                                 ...
                                                 String t = new String(rs.getString( index+1 ));
                                                 String t2 = new String(rs.getBytes(index+1),"UTF-8");
                                                 System.out.println("\n\n\ntype : "+info.m_Type+"\nvalue : "+t+" "+t2);*/
                ...
                protected int getProperties( Hashtable table, String id, boolean bInBase, String idRef1, String idRef2) throws WPSConnectorException
                {
                                ...
                                if( getColumnInfo( rs, id) && m_ColumnInfo != null)
                                {
                                        ...
                                        if( m_bMultipleRows && (maxCol > 0))
                                        {
                                                ...
                                                for( int i = 0; i < maxCol; ++i)
                                                        mainLst.add( new ArrayList());
                                                do
                                                {
                                                        for( int i = 0; i < maxCol; ++i)
                                                        {
                                                                ArrayList lst = (ArrayList) mainLst.get(i);
        ...
        // Type de la colonne de nommage des propri�t�s (pour la gestion de l'unicode dans un LONGVARBINARY)
        //private int m_ColumnNameType = java.sql.Types.OTHER;

        // Liste desrequ�tes des propri�t�s
        public ArrayList m_PropertyGroups = new ArrayList();
        ...
        public void openConnections( Hashtable wpsparams, Connection connection) throws WPSConnectorException
        {
                int size = m_PropertyGroups.size();
                for( int i = 0; i < size; i++)
                {
                        JDBCPropertyGroup grp = (JDBCPropertyGroup) m_PropertyGroups.get( i);
        ...
        public int getProperties( Hashtable table, String id, boolean bInBase, String idRef1, String idRef2) throws WPSConnectorException
        {
                int count = 0;
                int size = m_PropertyGroups.size();
                for( int i = 0; i < size; i++)
                ...
                if( !table.containsKey( "id")) table.put( "id", id);
                return count;
        }
}
