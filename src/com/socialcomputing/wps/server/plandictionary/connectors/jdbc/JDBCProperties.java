	...
	{ 
		...
		private boolean getColumnInfo(ResultSet rs, String id) throws WPSConnectorException {
			...
			try {
				...
				if (ret) {
					if (m_ColumnInfo == null) { // Recherche des types
						...
						for (int i = 1; i <= count; ++i) {
							...
							m_ColumnInfo.add(new InternalColumnInfo(
									m_bNameIsLabel ? rsmd.getColumnLabel(i)
				...
				throw new WPSConnectorException(
						"JDBCPropertyGroup failed to find column information for "
								+ m_Type + " '" + id + "'", e);
			}
			return ret;
		...
		private Object getProperty(ResultSet rs, int index) throws WPSConnectorException {
			...
			try {
				...
				case java.sql.Types.CHAR:
					 ...
					 * String testString =
					 * "ab\u3053\u3093\u306B\u3061\u306F\u4E16cd"; String t =
					 * new String(rs.getString( index+1 )); String t2 = new
					 * String(rs.getBytes(index+1),"UTF-8");
					 * System.out.println("\n\n\ntype : "
					 * +info.m_Type+"\nvalue : "+t+" "+t2);
					 */
				...
				default:
					return "void";
					// throw new WPSConnectorException(
					// "JDBCPropertyGroup unknown DB type: " + info.m_Type);
				}
			} catch (SQLException e) {
				throw new WPSConnectorException(
						"JDBCPropertyGroup unable to get column type", e);
			} catch (IOException e) {
				throw new WPSConnectorException("JDBCPropertyGroup error", e);
			}
		}

		protected int getProperties(Hashtable<String, Object> table, String id, boolean bInBase, String idRef1, String idRef2) throws WPSConnectorException {
			...
			try {
				...
				if (getColumnInfo(rs, id) && m_ColumnInfo != null) {
					int maxCol = (m_nbColumns == -1) ? m_ColumnInfo.size()
							: m_nbColumns;
					if (m_bMultipleRows && (maxCol > 0)) {
						...
						for (int i = 0; i < maxCol; ++i) {
							ArrayList<Object> lst = mainLst.get(i);
							if (lst.size() > 0) {
								InternalColumnInfo info = m_ColumnInfo
										.elementAt(i);
								switch (info.m_Type) {
								case java.sql.Types.BIGINT:
								case java.sql.Types.SMALLINT:
								...
								case java.sql.Types.INTEGER:
								case java.sql.Types.NUMERIC:
									table.put(info.m_Name, lst
											.toArray(new Integer[lst.size()]));
									break;
								case java.sql.Types.FLOAT:
								case java.sql.Types.REAL:
									table.put(info.m_Name,
											lst.toArray(new Float[lst.size()]));
									break;
								case java.sql.Types.DOUBLE:
									table.put(info.m_Name,
											lst.toArray(new Double[lst.size()]));
									break;
								case java.sql.Types.LONGVARBINARY:
								case java.sql.Types.LONGVARCHAR:
								case java.sql.Types.VARCHAR:
					...
					if (!m_bMultipleRows) {
						...
						for (int i = 0; i < maxCol; ++i) {
							...
							if (o != null) {
								InternalColumnInfo info = (InternalColumnInfo) m_ColumnInfo
										.elementAt(i);
				...
				throw new WPSConnectorException(
						"JDBCProperties failed to read properties", e);
			}
			return added;
		}
	}
		...
		if (element != null) {
			...
			for (int i = 0; i < size; ++i) {
				...
				if (node.getAttributeValue("column-name") != null)
					grp.m_bNameIsLabel = node.getAttributeValue("column-name")
							.equalsIgnoreCase("label");
				grp.m_bMultipleRows = node.getAttributeValue("multiple-rows")
						.equalsIgnoreCase("yes");
				try {
					grp.m_nbColumns = Integer.parseInt(node
							.getAttributeValue("columns-used"));
				} catch (Exception e) {
				}
				grp.m_bUseRefId1 = node.getAttributeValue("use-id1")
						.equalsIgnoreCase("yes");
				grp.m_bUseRefId2 = node.getAttributeValue("use-id2")
						.equalsIgnoreCase("yes");
