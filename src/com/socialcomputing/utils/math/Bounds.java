...
/**
 * Title: Plan Generator Description: classes used to generate a Plan that will
 * be sent to the applet. Copyright: Copyright (c) 2001 Company: Voyez Vous
 * 
 * @author
 * @version 1.0
 */
...
public class Bounds implements Serializable {
	...
	public void showHisto(String msg, int sCnt, float tresh, Object[] vals,
			ValueGetter getter) {
		showHisto(msg, sCnt, tresh, vals, 0, vals.length, getter);
	}

	public void showHisto(String msg, int sCnt, float tresh, Object[] vals,
			int beg, int end, ValueGetter getter) {
		int i, n;
	...
	public void clipBounds(int sCnt, float tresh, Object[] vals,
			ValueGetter getter) {
		clipBounds(sCnt, tresh, vals, 0, vals.length, getter);
	}

	public void clipBounds(int sCnt, float tresh, Object[] vals, int beg,
			int end, ValueGetter getter) {
		int i, n, max, h, hMax;
		int vCnt = end - beg;
		...
		while (n < vCnt) {
			isBeg = hBeg - 1 >= 0 && hEnd + 1 < sCnt ? histo[hBeg - 1] > histo[hEnd + 1]
					: hBeg - 1 >= 0;
	...
	public void adjustBounds(int sCnt, float tresh, Object[] vals, int beg,
			int end, ValueGetter getter) {
		int i, n, min, max, h;
		int vCnt = end - beg;
		int[] histo = new int[sCnt];
		...
		if (t > 1.f) {
			log.info("avg = {}, med = {}, var = {}",
					new Object[] { avg, med, t });
			...
			log.info("Clamped vals : {} < avg < {} over {}", new Object[] {
					min, max, vCnt });
		}
	}
}