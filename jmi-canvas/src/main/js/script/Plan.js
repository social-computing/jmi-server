...
JMI.script.Plan = ( function() {
	...
	Plan.prototype = {
		...
		initZones : function(gDrawingContext, zones, isFirst) {
			var i, n = zones.length, dim = this.applet.size;
		...
		updateZoneAt : function(p) {
			var cSat = JMI.script.Satellite, zone = JMI.script.ActiveZone, parent = this.curZone !== null ? this.curZone.getParent() : null, i;
		...
		updateCurrentZone : function(cSat, p) {
			...
			if(this.curZone !== this.newZone || this.curSat !== cSat) {
				...
				if(this.curZone !== null && (this.newZone === null || this.curZone.getParent() !== this.newZone.getParent())) {
					// Restore its rest image
					//ON rollover non active zone => redraw
					curZoneBounds = this.curZone.getParent().bounds;
