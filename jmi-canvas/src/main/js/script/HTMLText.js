...
JMI.script.HTMLText = ( function() {
	...
	HTMLText.prototype = {
		...
		initValues : function(inCol, outCol, textCol, fontSiz, fontStl, fontNam, blur, rounded, flags, margin) {
			...
			if((fontStl & JMI.script.HTMLText.BOLD ) !== 0) {
				this.font.bold = true;
			}
			if((fontStl & JMI.script.HTMLText.ITALIC ) !== 0) {
		...
		parseText : function(applet, gDrawingContext, htmlText) {
			...
			var tokenStr, nextStr, prevStr = 0 < tokenizer.length ? tokenizer[0] : null;
			var hasMore = 1 < tokenizer.length, isText = false;
			...
			this.updateTag(applet, gDrawingContext, "br");
			// to set last line position
		...
		updateBounds : function() {
			...
			var i, n = this.tokens.length, x = 0, y = margin.top;
			var token;
			var tTok = null;
			var fTok = null;
		...
		updateText : function(gDrawingContext, text, textTok, isText) {
			...
			if(text.length > 0) {
				...
				if(isText) {
					...
					if(textTok.bounds.height < h) {
						textTok.bounds.height = h;
					}
				}
		...
		updateTag : function(applet, gDrawingContext, tag) {
			...
			if(tag.length > 0) {
				...
				if(begChar === '/') {
					...
					{
						textTok = this.closeTag(applet, gDrawingContext, tempTag);
						if(nxtChar !== 'p') {
							return textTok;
						}
					} else {
				...
				var prevMrg = prevTok.margin, margin = this.body.margin;
				var flags = this.body.flags, width = this.wCur + (prevMrg !== null ? prevMrg.left + prevMrg.right : 0 );
				...
				if(tag === "br" || tag === "br/" || begChar === 'p' || tag === "/p") {
					...
					if(begChar === 'p') {
						var alignStr = this.readAtt(tag, "a");

						if(alignStr !== null) {
							var align = alignStr.charAt(0).toLowerCase();
							flags = align === 'r' ? JMI.script.HTMLText.RIGHT_BIT : (align === 'c' ? JMI.script.HTMLText.CENTER_BIT : 0 );
						}
						margin = this.readMargin(tag);

						if(tag.length > 1 && alignStr === null && margin === null) {
							flags = this.body.flags;
							applet.log("[updateTag] syntax error Tag : " + tag);
							return null;
						} else {
							this.heap.addElement(tag);
						}
					}

					// update previous format Token
		...
		closeTag : function(applet, gDrawingContext, tag) {
			...
			this.removeElement(this.heap, tag);

			if(c === 'b') {
				this.style &= ~JMI.script.Font.BOLD;
			} else if(c === 'i') {
				this.style &= ~JMI.script.Font.ITALIC;
			}
			...
			if(this.isGfx(c)) {
				while(i-- > 0) {// find previous Tag of the same type in the heap
					...
					if(prevTag.charAt(0) === c) {
						return this.updateGfx(applet, gDrawingContext, prevTag);
					}
				}
			}
			textTok.font = this.font;
		...
		drawText : function(gDrawingContext, size, dir) {
			...
			var xMax = size.width - this.bounds.width - 1, yMax = size.height - this.bounds.height - 1;
		...
		setTextBnds : function(size, flags, posFlags, transfo, supCtr, center) {
			var isFloat = JMI.script.Base.isEnabled(flags, JMI.script.HTMLText.FLOAT_BIT), dx = 0, dy = 0, x = center.x, y = center.y, w = this.bounds.width, h = this.bounds.height, w2 = w >> 1, h2 = h >> 1;
 ...
 */
JMI.script.HTMLText.CORNER_BIT = 0x0100;
// Be carefull with this flags, they must not override Fonts ones (0x1, 0x2)!
 ...
 */
JMI.script.HTMLText.NORTH = 0x4;
// 0 1 0 0;
JMI.script.HTMLText.NORTH_EAST = 0x8;
// 1 0 0 0;
JMI.script.HTMLText.EAST = 0x9;
// 1 0 0 1;
JMI.script.HTMLText.SOUTH_EAST = 0xA;
// 1 0 1 0;
JMI.script.HTMLText.SOUTH = 0x6;
// 0 1 1 0;
JMI.script.HTMLText.SOUTH_WEST = 0x2;
// 0 0 1 0;
JMI.script.HTMLText.WEST = 0x1;
// 0 0 0 1;
JMI.script.HTMLText.NORTH_WEST = 0x0;
// 0 0 0 0;

JMI.script.HTMLText.BOLD = 0x1;
// 0 1 0 0;
JMI.script.HTMLText.ITALIC = 0x2;
// 0 1 0 0;
...
JMI.script.TextToken = ( function() {
	...
	TextToken.prototype = {
		...
		paint : function(gDrawingContext, pos, borderWidth) {
			var x = this.bounds.x + pos.x, y = this.bounds.y + pos.y;
