 ...
 * @version 1.0
 */
JMI.namespace("com.socialcomputing.jmi.script.Slice") = (function() {
/**
 * A buffer to store the HTMLText associated with this if it has one.
 */
	var m_htmlTxt, //:HTMLText;
	Constr;
	
	Constr = function() {
	}
	Constr.prototype = {
		constructor: com.socialcomputing.jmi.script.Slice,
		version: "2.0"
	}
	return Constr;
}());
 ...
 */
com.socialcomputing.jmi.script.Slice.TRANSFO_VAL = 1;

/**
 * Index of the inside Color prop in VContainer table.
 */
com.socialcomputing.jmi.script.Slice.IN_COL_VAL = 2;

/**
 * Index of the border Color prop in VContainer table.
 */
com.socialcomputing.jmi.script.Slice.OUT_COL_VAL = 3;

/**
 * Index of the image URL prop in VContainer table.
 */
com.socialcomputing.jmi.script.Slice.IMAGE_VAL = 4;

/**
 * Index of the text (HTML or not) prop in VContainer table.
 */
com.socialcomputing.jmi.script.Slice.TEXT_VAL = 5;

/**
 * Index of the text (HTML or not) prop in VContainer table.
 */
com.socialcomputing.jmi.script.Slice.ALPHA_VAL = 6;

/**
 * Index of the delay (ms) prop for a tooltip Slice.
 */
com.socialcomputing.jmi.script.Slice.DELAY_VAL = 7;

/**
 * Index of the length (ms) prop for a tooltip Slice.
 */
com.socialcomputing.jmi.script.Slice.LENGTH_VAL = 8;

/**
 * True if this Slice is visible.
 * Probably a useless bit...
 */
com.socialcomputing.jmi.script.Slice.VISIBLE_BIT = 0x01;
 ...
 */
com.socialcomputing.jmi.script.Slice.prototype.paint = function(applet, s, supZone, zone, satShp, satCtr, supCtr) {
	var text = getText( TEXT_VAL, zone.m_props );
    
    var transfo = getTransfo( TRANSFO_VAL, zone.m_props );
    ...
    if(isDefined(IMAGE_VAL)) {
		var imageNam = parseString(IMAGE_VAL, zone.m_props )[0];
        if (imageNam != null) {
    ...
    if(text != null) {
        ...
        {
			m_htmlTxt = null;
            var textUrls  = text.parseString( HTMLText.TEXT_VAL, zone.m_props );
            var t , hTxt = "";
            var i = 0;
            // TODO portage
 ...
 */
com.socialcomputing.jmi.script.Slice.prototype.contains = function(planComponent, g, supZone, zone, satShp, satCtr, supCtr, pos){
	var transfo = getTransfo(TRANSFO_VAL, zone.m_props);
 ...
 */
com.socialcomputing.jmi.script.Slice.prototype.setBounds = function(applet, g, supZone, zone, satShp, satCtr, supCtr, bounds) {
    var transfo = getTransfo(TRANSFO_VAL, zone.m_props);
    ...
    try {
        ...
        if ( supZone != null )
        {
            var points = satShp.getValue( ShapeX.POLYGON_VAL, supZone.m_props );
