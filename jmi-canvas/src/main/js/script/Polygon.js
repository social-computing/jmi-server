...
JMI.script.Polygon = ( function() {
	...
	Polygon.prototype = {
		...
		containsCrossing : function(p) {
			...
			for( i = 0; i < this.npoints; i++) {
				...
				if(((this.ypoints[i] <= p.y) && (this.ypoints[i2] > p.y)) || ((this.ypoints[i] > p.y) && (this.ypoints[i2] <= p.y))) {
					...
					vt = (p.y - this.ypoints[i]) / (this.ypoints[i2] - this.ypoints[i]);
		...
		calculateBounds : function(xpoints, ypoints, npoints) {
			var minPoint = new JMI.script.Point(xpoints[0], ypoints[0]), maxPoint = new JMI.script.Point(xpoints[0], ypoints[0]), i;
			...
			this.bounds = new JMI.script.Rectangle(minPoint.x, minPoint.y, maxPoint.x - minPoint.x, maxPoint.y - minPoint.y);
			return this;
		}
	};
