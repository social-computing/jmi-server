...
JMI.script.Swatch = ( function() {
	...
	Swatch.prototype = {
		...
		drawSats : function(applet, gDrawingContext, zone, shape, transfo, isLinkOnly, isCur, isFront, showTyp) {
			...
			var curZone = applet.planContainer.map.plan.curZone, subZone;
			var sat = this.satellites[0];
			var satData = isCur ? zone.curData[0] : zone.restData[0];
			var satRelTrf, satTrf;
			var i, j, n = this.satellites.length, flags, dir;
			var hasRestBit, hasCurBit, isCurSub;
			var satCtr, supCtr = shape.getCenter(zone);
			...
			for( i = 1; i < n; i++) {
				...
				if(((isLinkOnly && JMI.script.Base.isEnabled(flags, JMI.script.Satellite.LINK_BIT)) || !isLinkOnly) && JMI.script.Base.isEnabled(flags, JMI.script.Satellite.VISIBLE_BIT)
				// This Sat is visible
				&& (isFront !== JMI.script.Base.isEnabled(flags, JMI.script.Satellite.BACK_BIT))) {
					...
					if(isBag) {
						...
						if(supZone.dir !== 10.0) {
							if(!JMI.script.Base.isEnabled(flags, JMI.script.Satellite.NOSIDED_BIT)) {
								satTrf.direction = supZone.dir;
							} else {
								if(JMI.script.Base.isEnabled(supZone.flags, JMI.script.ActiveZone.LEFT_BIT)) {
									satTrf.direction += Math.PI;
								}
							}
						}

						dir = satTrf.direction;
		...
		getBounds : function(applet, gDrawingContext, zone, isCurZone) {
			...
			var zones = isBag ? supZone.subZones : null;
			var subZone;
			var satRelTrf, satTrf, transfo = sat.getTransfo(JMI.script.Satellite.TRANSFO_VAL, zone.props);
			var i, j, n = this.satellites.length, flags;
			...
			for( i = 1; i < n; i++) {
				...
				if(JMI.script.Base.isEnabled(flags, JMI.script.Satellite.VISIBLE_BIT)) {
					...
					if(isBag) {
						...
						if((!JMI.script.Base.isEnabled(flags, JMI.script.Satellite.SEL_BIT) || satData.isVisible) && JMI.script.Base.isEnabled(flags, JMI.script.Satellite.SUPER_BIT)) {
							satCtr = shape.transformOut(zone, satTrf);
		...
		getSatAt : function(planComponent, g, zone, pos, isCurZone) {
			...
			var sat, shape, isBag, supZone, zones, curZone, subZone, satRelTrf, satTrf,
				transfo, i, n, flags, hasRestBit, hasCurBit, hasSubBit, isCur,
				isVisible, satData, supCtr;

			if(parentzone.bounds.contains(pos.x, pos.y)) {
				sat = this.satellites[0];
				shape = sat.shapex;
				isBag = zone instanceof JMI.script.BagZone;
				supZone = isBag ? zone : null;
				zones = isBag ? supZone.subZones : null;
				curZone = planComponent.planContainer.map.plan.curZone;
				transfo = sat.getTransfo(JMI.script.Satellite.TRANSFO_VAL, zone.props);
				n = this.satellites.length;
				...
				for( i = n - 1; i > 0; i--) {
					...
					if(JMI.script.Base.isEnabled(flags, JMI.script.Satellite.VISIBLE_BIT) && (isCurZone || !JMI.script.Base.isEnabled(flags, JMI.script.Satellite.TIP_BIT))) {
						...
						if(isBag) {
							...
							if(JMI.script.Base.isEnabled(flags, JMI.script.Satellite.SUPER_BIT)) {
								...
								if(isVisible && ((hasRestBit && !isCur) || (hasCurBit && isCur)) && sat.contains(planComponent, g, zone, shape.transformOut(zone, satTrf), supCtr, satTrf, pos, false, false)) {
									return sat;
								}
							}
							...
							if(zones !== null && hasSubBit) {
								...
								for( j = m - 1; j >= 0; j--) {
									...
									if(isVisible && ((hasRestBit && !isCur) || (hasCurBit && isCur)) && sat.contains(planComponent, g, subZone, shape.transformOut(zone, satTrf), supCtr, satTrf, pos, false, false)) {
										return sat;
									}
								}
							}
						...
						// Else, if it's a link zone
							...
							if(isVisible && sat.contains(planComponent, g, zone, null, null, transfo, pos, false, true)) {
								return sat;
							}
						}
					}
				...
				sat = this.satellites[0];
				if(sat.contains(planComponent, g, zone, null, null, transfo, pos, zones !== null, true) || (isCurZone && !( zone instanceof JMI.script.LinkZone))) {
					return sat;
				}
			}
			return null;
		...
		evalSatData : function(applet, zone, isSuper) {
			var satDatas = [];
			var satData, 
			isTip, isSel, i, sat, flags, sels, sel;
			...
			for(i = 0; i < this.satellites.length; i++) {
				...
				if(isTip || isSel) {
					sels = sat.parseString(JMI.script.Satellite.SELECTION_VAL, zone.props);
					sel = -1;
